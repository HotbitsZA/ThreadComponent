...
namespace
{
  using HttpsServerType = SimpleWeb::Server<SimpleWeb::HTTPS>;
  ...
  [[nodiscard]] bool hasHeader(
      const cHTTPSServer::td_Fields &headers,
      std::string_view headerName)
  {
    return std::any_of(
        headers.begin(),
        headers.end(),
        [headerName](const cHTTPSServer::st_Field &header)
        {
  ...
  [[nodiscard]] cHTTPSServer::td_Fields copyFields(const MultiMapType &fields)
  {
    cHTTPSServer::td_Fields copied;
  ...
  [[nodiscard]] cHTTPSServer::st_Request makeRequestSnapshot(
      const std::shared_ptr<HttpsServerType::Request> &request)
  {
    cHTTPSServer::st_Request snapshot;
    if (!request)
    {
      return snapshot;
    }
  ...
  void writeResponse(
      const std::shared_ptr<HttpsServerType::Response> &response,
...
struct cHTTPSServer::cResponse::st_State
{
...
struct cHTTPSServer::st_Impl
{
  ...
  std::unique_ptr<HttpsServerType> server;
...
cHTTPSServer::cResponse::cResponse(std::shared_ptr<st_State> state)
    : m_state(std::move(state))
{
}

cHTTPSServer::cResponse::~cResponse()
{
...
void cHTTPSServer::cResponse::write(std::string_view body) const
{
  write(200U, body, {});
}

void cHTTPSServer::cResponse::write(
    std::uint16_t statusCode,
    std::string_view body,
...
void cHTTPSServer::cResponse::redirect(
...
bool cHTTPSServer::cResponse::valid() const noexcept
{
      ...
      m_impl(std::make_unique<st_Impl>())
{
  ...
  m_impl->certificateFile = std::move(certificateFile);
  m_impl->privateKeyFile = std::move(privateKeyFile);
  m_impl->verifyFile = std::move(verifyFile);
  m_impl->bindAddress = std::move(bindAddress);
  m_impl->threadPoolSize = (threadPoolSize == 0U) ? 1U : threadPoolSize;
}

cHTTPSServer::~cHTTPSServer() noexcept
{
  (void)stopThread();
}

bool cHTTPSServer::addResource(
...
bool cHTTPSServer::setDefaultResource(
    const std::string &method,
    td_RequestHandler handler)
{
  const auto normalizedMethod = normalizeMethod(method);
...
void cHTTPSServer::setErrorHandler(td_ErrorHandler handler)
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::clearErrorHandler()
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setBindAddress(std::string bindAddress)
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setPort(std::uint16_t port) noexcept
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setThreadPoolSize(std::size_t threadPoolSize) noexcept
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setReuseAddress(bool reuseAddress) noexcept
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setFastOpen(bool fastOpen) noexcept
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setRequestTimeoutSeconds(long timeoutSeconds) noexcept
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setContentTimeoutSeconds(long timeoutSeconds) noexcept
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setMaxRequestSize(std::size_t maxRequestSize) noexcept
{
  if (isRunning())
  {
    return;
...
void cHTTPSServer::setCertificateFiles(
    std::string certificateFile,
    std::string privateKeyFile,
    std::string verifyFile)
{
...
std::string cHTTPSServer::bindAddress() const
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->bindAddress;
}

std::string cHTTPSServer::certificateFile() const
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->certificateFile;
}

std::string cHTTPSServer::privateKeyFile() const
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->privateKeyFile;
}

std::string cHTTPSServer::verifyFile() const
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->verifyFile;
}

std::uint16_t cHTTPSServer::configuredPort() const noexcept
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->configuredPort;
}

std::uint16_t cHTTPSServer::listeningPort() const noexcept
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->listeningPort;
}

std::size_t cHTTPSServer::threadPoolSize() const noexcept
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->threadPoolSize;
}

std::string cHTTPSServer::lastError() const
{
  std::lock_guard<std::mutex> lock(m_impl->mutex);
  return m_impl->lastError;
}

bool cHTTPSServer::startThread(duration_type waitForStartTimeout)
{
...
bool cHTTPSServer::startThread(std::uint16_t waitForStartTimeoutMilliSec)
{
  return startThread(duration_type{waitForStartTimeoutMilliSec});
}

bool cHTTPSServer::preRun()
{
  try
  {
...

    {
      ...
      certificateFile = m_impl->certificateFile;
      privateKeyFile = m_impl->privateKeyFile;
      verifyFile = m_impl->verifyFile;
    }
    ...
    if (certificateFile.empty() || privateKeyFile.empty())
    {
      m_impl->resolveStartIfPending(
          false,
          0U,
          "HTTPS server requires certificate and private key files");
      return false;
    }

    {
      std::lock_guard<std::mutex> lock(m_impl->mutex);
      m_impl->server =
          std::make_unique<HttpsServerType>(certificateFile, privateKeyFile, verifyFile);
    ...
    for (const auto &route : resources)
    {
      m_impl->server->resource[route.pathRegex][route.method] =
          [this, handler = route.handler, errorHandler](
              std::shared_ptr<HttpsServerType::Response> response,
              std::shared_ptr<HttpsServerType::Request> request)
      {
        updateHeartbeat();
        ...
        {
          const std::string message = "Unhandled HTTPS request handler exception";
    ...
    for (const auto &route : defaultResources)
    {
              ...
              std::shared_ptr<HttpsServerType::Request> request)
      {
        ...
        {
          const std::string message = "Unhandled HTTPS default handler exception";
          m_impl->setLastError(message);
          if (errorHandler)
          {
            errorHandler(requestSnapshot, -1, message);
  ...
  {
    m_impl->resolveStartIfPending(false, 0U, "Failed to initialize HTTPS server");
  }
...
void cHTTPSServer::run()
{
  try
  {
    HttpsServerType *server = nullptr;
    {
      std::lock_guard<std::mutex> lock(m_impl->mutex);
    ...
    if (server == nullptr)
    {
      m_impl->resolveStartIfPending(false, 0U, "HTTPS server is not initialized");
      return;
    }
  ...
  {
    m_impl->resolveStartIfPending(false, 0U, "Unhandled HTTPS server exception");
    throw;
  }
...
void cHTTPSServer::stopTriggered()
{
  HttpsServerType *server = nullptr;
  {
    std::lock_guard<std::mutex> lock(m_impl->mutex);
  ...
  m_impl->resolveStartIfPending(false, 0U, "HTTPS server stop requested");
}

bool cHTTPSServer::waitUntilListening(duration_type waitForStartTimeout)
{
