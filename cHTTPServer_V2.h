...
/**
  ...
  Direct Simple-Web-Server wrapper integrated with cBaseWorker_V2.
...
*/
class cHTTPServer_V2 : public cBaseWorker_V2
{
public:
  using server_type = SimpleWeb::Server<SimpleWeb::HTTP>;
  ...
  explicit cHTTPServer_V2(
      std::string name,
      std::uint16_t port = 0U,
      std::size_t ioThreadCount = 1U);
  ~cHTTPServer_V2() noexcept override;

  cHTTPServer_V2(const cHTTPServer_V2 &) = delete;
  cHTTPServer_V2 &operator=(const cHTTPServer_V2 &) = delete;
  cHTTPServer_V2(cHTTPServer_V2 &&) = delete;
  cHTTPServer_V2 &operator=(cHTTPServer_V2 &&) = delete;
